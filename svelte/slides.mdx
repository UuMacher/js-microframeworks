export { theme } from '../theme';

import { CodeSurfer } from "mdx-deck-code-surfer"

# Svelte 3.0

<blockquote cite="https://svelte.dev/">Cybernetically enhanced web apps</blockquote>

---

## Features

- Svelte = Compiler
- _No runtime_
- Write less code
- Truly reactive
- No virtual DOM
- Surgically updates the DOM
- Vanilla JS
- *fast* (faster than Angular, Ember, React, Preact, Riot, Mithrill, ...)

---

## Principles

- Declarative components (.svelte, single file)
    - JavaScript, HTML, CSS
    - Special syntax for reactive statements
- Svelte converts components into plain JavaScript

---

## Architecture

<img src="./images/architecture.svg" alt="Architecture of svelte visualized" />

---

## Getting started
### Use the REPL (Read-eval-print loop)

https://svelte.dev/repl

---
## Getting started
### Use degit / svelte-template
```
npx degit sveltejs/template my-svelte-project
cd my-svelte-project
npm install
npm run dev
```
---
## Getting started
### Use a bundler 
- rollup (https://github.com/rollup/rollup-plugin-svelte)
- webpack (https://github.com/sveltejs/svelte-loader)

---

## Hello component

### Demo

---

## Defining the Hello component

<CodeSurfer
  title="hello.svelte"
  code={require("!raw-loader!./examples/hello-world/hello.svelte").default}
  lang="javascript"
  showNumbers={false}
  dark={true}
  steps={[
    { lines: [5], notes: "Basic template" },
    { range: [1, 3] , notes: "Define external properties"},
    { notes: "Hello World" }
  ]}
/>

---

## Using the Hello component

<CodeSurfer
  title="index.js"
  code={require("!raw-loader!./examples/hello-world/index.js").default}
  lang="javascript"
  showNumbers={false}
  dark={true}
  steps={[
    { lines: [2], notes: "Import the component" },
    { lines: [4, 10] , notes: "Create a new Instance of the component"},
    { lines: [5, 6] , notes: "Pass the DOM mounting point"},
    { lines: [7, 8, 9] , notes: "Pass the properties"},
    { notes: "Profit! 🍾" }
  ]}
/>